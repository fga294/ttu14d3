...
export default function DataTable({ data, columns, onRowClick, initialSorting = [] }) {
        ...
        <thead>
          <tr style={{ background: "var(--surface-700)", borderBottom: "1px solid var(--surface-600)" }}>
            {table.getHeaderGroups().map((hg) =>
              hg.headers.map((h) => {
                const highlight = h.column.columnDef.meta?.highlight;
                return (
                  <th
                    key={h.id}
                    ...
                    style={{
                      ...
                      background: highlight ? "rgba(255, 215, 0, 0.08)" : undefined,
                    }}
